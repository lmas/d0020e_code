...
func Test_set_minTemp(t *testing.T) {
	...
	/*
		...
		w := httptest.NewRecorder()
		body := bytes.NewReader([]byte(`{"value": 20, "unit": "Celsius", "version": "SignalA_v1.0"}`))          // converts the Jason data so it can be read
		r := httptest.NewRequest("PUT", "http://localhost:8670/Comfortstat/Set%20Values/min_temperature", body) // simulating a put request from a user to update the min temp
		...
		log.Printf("Response Body: %s", string(respbody))
			...
			value := strings.Contains(string(respbody), `"value": 20`)
			unit := strings.Contains(string(respbody), `"unit": "Celsius"`)
			version := strings.Contains(string(respbody), `"version": "SignalA_v1.0"`)
...
func Test_set_minPrice(t *testing.T) {
	...
	w := httptest.NewRecorder()
	r := httptest.NewRequest("GET", "localhost:8670/Comfortstat/Set%20Values/max_price", nil)
	good_statuscode := 200
	ua.set_minPrice(w, r)
	...
	// force the case to hit default statement but alter the method
	w = httptest.NewRecorder()
	r = httptest.NewRequest("666", "localhost:8670/Comfortstat/Set%20Values/max_price", nil)
